...
**Spacing belongs to the parent, never to the child** (added 2026-08-17). A panel that carries its own `mt-*` works only for as long as it is stacked; drop it into a grid cell and that margin fights the grid's `gap`, offsetting it from the panel beside it. The night page shipped exactly that bug — a door-sheet panel sitting 12px below the Sports panel in the same row, for no reason visible in either component. Where a panel is placed by a grid, it carries no outer margin at all.

The dashboard's panels do still carry `mt-3` themselves, because they are stacked siblings there and the margin is doing real work. That is a convention worth converging on a `space-y-*` wrapper eventually; until then, the rule to apply is that **a component destined for a grid cell must be margin-free**, and a component that might go either way should take its spacing from a `className` prop rather than baking one in.
